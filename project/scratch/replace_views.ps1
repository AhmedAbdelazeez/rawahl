$content = [System.IO.File]::ReadAllText("Pages\Index.cshtml", [System.Text.Encoding]::UTF8)

$startToken = '<!-- ================= أولاً: شاشة الملخص العام والتفصيل الكلي (Overview View) ================= -->'
$endToken = '<!-- ================= ثالثاً: شبكة الإدارات والأقسام (Departments Grid View) ================= -->'

$startIndex = $content.IndexOf($startToken)
if ($startIndex -lt 0) {
    Write-Error "Start token not found"
    exit 1
}

$endIndex = $content.IndexOf($endToken)
if ($endIndex -lt 0) {
    Write-Error "End token not found"
    exit 1
}

$newViews = @"
<!-- ================= أولاً: شاشة الملخص العام والتفصيل الكلي (Overview View) ================= -->
